...
const PlaceBtn = ({ color, number, isBorderBt, isNone, onTap, routeTur, deck }) => {
  ...
  const styles = {
    wrapper: {
      backgroundColor: isBooked ? 'gray' : isBookedAdmin ? 'gray' : color,
      borderBottomWidth: isBorderBt ? 1 : 0,
      zIndex: isNone ? -1 : 1,
    },
  }
  const handler = () => {
    if (!(isNone || isBooked || isBookedAdmin)) onTap(number, color, deck)
  }
